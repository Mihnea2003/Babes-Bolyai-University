...
void resize(DynamicArray* arr)
{
    ...
    arr->capacity *= 2;
    void** auxArr = (void**)realloc(arr->elements, sizeof(void*) * arr->capacity);
    if (auxArr == NULL) {
        return;
    }
    arr->elements = auxArr;
...
{
    Material* material = createMaterial("Faina", "Bunica", 100, 5);
    DynamicArray* arr = createDynamicArray(10, (DestroyElementFunction)destroyMaterial, (CopyElementFunction)copyMaterial);
    addElement(arr, material);
    assert(arr->length== 1);

    assert(getArrayLength(arr)== 1);

    Material* material1 = (Material*)getElementAtIndex(arr, 0);
    assert(material->supplier== material1->supplier);
