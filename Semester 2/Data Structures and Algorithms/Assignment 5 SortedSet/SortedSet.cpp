...
//always Theta(1)
TComp SortedSet::getElement(int position) {
    if (position < 0 || position >= arrayCapacity || elements[position].info == NULL_TELEM) {
        // Position is invalid or contains a null element
        throw std::exception();
    }

    return elements[position].info;
}
//best case,where every element returns true after condition,Theta(n)
//worst case,where every element returns false after condition ,Theta(n^2)
//Overall case O(n^2)
void SortedSet::filter(Condition cond) {
    for (int i=0; i<this->size();i++){
        if(!cond(this->getElement(i))){
            this->remove(getElement(i));
        }
    }
}
